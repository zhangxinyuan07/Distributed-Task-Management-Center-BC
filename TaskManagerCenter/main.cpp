...
int main(int argc, char *argv[])
{
    ...
    // 记录sockfd对应的地址,方便下面还原时用
    unordered_map<int, string> sockfdToAddr;
    ...
    while (true)
    {
        ...
        for (int i = 0; i < fd_num; i++)
        {
            ...
            if (sockfd == listenfd)
            {
                ...
                // 将新客户的数据初始化,并放入用户数组
                users[connfd].init(connfd, client_addr, sockfdToAddr);
            }
            ...
            {
                // 这里每当并发连接第二个客户端时
                // 就会有users数组里保存的CWorkLogic对象的m_sockfd变成0, m_cliIP变成空字符串
                // 为了解决这个问题, 在这里还原一下
                users[sockfd].m_sockfd = sockfd;
                users[sockfd].m_cliIP = sockfdToAddr[sockfd];
                // 有读事件到达, 一次性读出所有数据
